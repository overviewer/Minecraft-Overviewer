...
class World:
  ...
  def getChunk(self, x, z, dimension=0):
    ...
    if chunk is None:
      ...
      # TODO: Too much memory used for caching all chunks
      # A 700mb Bedrock map save used 20+gb of memory
      # I'm not using this cache because I think Overviewer has it's own smart cache for recently accessed chunks as well
      # We may need to revisit this to improve performance
  ...
  def iterChunks(self, start=None, end=None, dimension=0, coordinatesonly=False):
    if not self.chunkcoordinates:
      self.cacheChunkCoordinates()

    for x, z, dim in sorted(self.chunkcoordinates.keys()):
      if dim != dimension:
        continue
  ...
  def cacheChunkCoordinates(self):
    for k in ldb.iterateKeys(self.db):
      # Overworld
      if len(k) == 9 and k.endswith((b"v", b",")):
        x, z = struct.unpack("<ii", k[:8])
        if (x, z, 0) not in self.chunkcoordinates:
          self.chunkcoordinates[(x, z, 0)] = True
      # Nether / The End
      if len(k) == 13 and k.endswith((b"v", b",")):
        x, z, dim = struct.unpack("<iii", k[:12])
        if (x, z, dim) not in self.chunkcoordinates:
          self.chunkcoordinates[(x, z, dim)] = True

  def chunkExists(self, x, z, dimension=0):
    if (x, z, dimension) in self.chunks:
        return True

    keyBase = None
    if dimension == 0:
      keyBase = struct.pack("<ii", x, z)
    else:
      keyBase = struct.pack("<iii", x, z, dimension)

    try:
      ldb.get(self.db, keyBase + b",")
      return True
    except KeyError:
      try:
        ldb.get(self.db, keyBase + b"v")
        return True
      except KeyError:
        return False
...
class Chunk:
  ...
  def _loadTileEntities(self, db):
    ...
    while not data.finished():
      ...
      if not block:
        logging.debug("Cannot apply nbt to block at {} {} {} since it does not exist.".format(x, y, z))
        continue
      block.nbt = nbtData
