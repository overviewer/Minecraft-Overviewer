...
def _build_full_block(top, side1, side2, side3, side4, bottom=None, blockID=None):
    ...
    A non transparent block uses top and sides 3 and 4
...
def generate_special_texture(blockID, data):
    ...
    if blockID == 55: # redstone wire
        ...
        if data & 0b1000000 == 64: # powered redstone wire
            redstone_cross_t = terrain_images[84+16]
            redstone_wire_t = terrain_images[85+16]        
        else: # unpowered redstone wire
            redstone_cross_t = terrain_images[84]
            redstone_wire_t = terrain_images[85]
...
special_blocks = set([66,59,61,62, 65,64,71,91,86,2,18,85,17,23,35,51,9,55])
